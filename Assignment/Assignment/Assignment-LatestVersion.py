...
def calculateCycles(instruction, registerLower, registerHigher, registerNames):
    ...
    if len(instruction) == 3:
        ...
        if instruction[0] == 'MOV':
            ...
            instructionNames = 'Fetch, Decode, Execute'
            return 3, instructionNames
        elif instruction[0] == "MUL" or instruction[0] == "DIV" or instruction[0] == "AND" or instruction[0] == "OR" or instruction[0] == "XOR":
            globals()['ALU'] = 1
            instructionNames = 'Fetch, Decode, Execute'
            return 3, instructionNames
        elif instruction[0] == 'CMP' or instruction[0] == 'JMP' or instruction[0] == 'ROR' or instruction[0] == 'ROL':
            globals()['ALU'] = 1
            instructionNames = 'Fetch, Decode, Execute'
            return 3, instructionNames
    elif len(instruction) == 2:
        if instruction[0] == 'INC' or instruction[0] == 'DEC' or instruction[0] == 'SHR' or instruction[0] == 'SHL' or instruction[0] == 'NOT':
            globals()['ALU'] = 1
            instructionNames = 'Fetch, Decode, Execute'
            return 3, instructionNames            
...
def main(machineCode, registers, memoryLocations, instructions, registerLower, registerHigher, registerNames):
    ex = exit.get()
    if ex == 'y':
        sys.exit()
...
def mainMenu():
    ...
    while True:
        ...
        label = Label(root1,text="Exit?", font=('Times New Roman',17,'bold')).grid(row=1,column=0)
        exit = Entry(root1, bg = 'Black', fg = 'white', font=('Times New Roman',17,'bold'))
        exit.grid(row=1,column=1)
        button = Button(root1,text="Enter", width = 12, height = 3, font=('Times New Roman',17,'bold'), command = lambda : main(machineCode, registers, memoryLocations, instructions, registerLower, registerHigher, registerNames)).grid(row=3,column=0,columnspan=3, sticky = W + E)
        mainloop()
